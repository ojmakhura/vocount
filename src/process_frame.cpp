...
int chooseMinPts(map<uint, set<int>>& numClusterMap, vector<int>& validities){
	...
	while(!found){
		...
		if(!found){
			...
			/*
			cout << "............................................................................. : " << endl;
			printf("DEBUG: Tmp has size %ld and is \n", tmp.size());
			for(map<uint, set<int>>::iterator it = tmp.begin(); it != tmp.end(); ++it){
...
selection_t detectColourModel(Mat& frame, Mat& descriptors, vector<KeyPoint>& keypoints, ofstream& trainingFile){
    ...
    //if(trainingFile.is_open()){
		//trainingFile << "minPts,Num of Clusters, Validity" << endl;   
	//}
...
void trackFrameColourModel(vocount& vcount, framed& f, selection_t& colourSel, Mat& frame){
	...
	for (set<int32_t>::iterator itt = colourSel.selectedClusters.begin(); itt != colourSel.selectedClusters.end(); ++itt) {
		...
		for(int32_t x = 0; x < list->size; x++){
			...
			//if(newCluster != 0){
				//currSelClusters.insert(newCluster);			
			//}
		}
